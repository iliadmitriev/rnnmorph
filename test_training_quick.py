...
# -*- coding: utf-8 -*-
"""
Quick training test - 1 epoch on 20K words to verify setup works.
"""
...
def create_test_configs():
    # Build config - standard architecture
    ...
    build_config.use_pos_lm = True  # Auxiliary POS language modeling
    build_config.use_word_lm = False
    
    build_config.save('rnnmorph/datasets/prepared/test_build_config.json')
    
    # Train config - 1 epoch, small batches for testing
    ...
    # Use simpler bucket borders for small test data
    train_config.sentence_len_groups = ((1, 10), (10, 20), (20, 40))
    
    # Set output paths - use ORIGINAL model vectorizers!
    ...
    # Use ORIGINAL model vectorizers (already built with all tags)
    train_config.gram_dict_input = 'rnnmorph/models/ru/gram_input.json'
    train_config.gram_dict_output = 'rnnmorph/models/ru/gram_output.json'
    train_config.word_vocabulary = 'rnnmorph/models/ru/word_vocabulary.pickle'
    train_config.char_set_path = 'rnnmorph/models/ru/char_set.txt'
...
if __name__ == '__main__':
    print("="*70)
    print("RNNMorph Training Test - 1 Epoch on 20K Words")
    print("="*70)
    ...
    # Training data
    train_file = 'rnnmorph/datasets/prepared/training_test_varied.txt'
    print(f"\nTraining data: {train_file}")
    
    # Count data
    with open(train_file, 'r') as f:
        lines = f.readlines()
        sentences = sum(1 for l in lines if l.strip() == '')
    ...
    output_files = [
        'rnnmorph/datasets/prepared/test_train_model.json',
        'rnnmorph/datasets/prepared/test_train_model.h5',
        'rnnmorph/datasets/prepared/test_eval_model.json',
        'rnnmorph/datasets/prepared/test_eval_model.h5',
        'rnnmorph/datasets/prepared/test_gram_input.json',
        'rnnmorph/datasets/prepared/test_gram_output.json',
        'rnnmorph/datasets/prepared/test_word_vocabulary.pickle',
        'rnnmorph/datasets/prepared/test_char_set.txt',
    ]
    
    for f in output_files:
        exists = "✓" if os.path.exists(f) else "✗"
        size = os.path.getsize(f) / 1024 if os.path.exists(f) else 0
        print(f"  {exists} {f.split('/')[-1]:<40} ({size:.1f} KB)")
